...
public class Problem1367 {
    ...
    public boolean isSubPath(ListNode head, TreeNode root) {
        ...
        Set<TreeNode> noChooseSet = new HashSet<>();
        Set<TreeNode> chooseSet = new HashSet<>();

        return find(head, indexNode, root, true, false, chooseSet, noChooseSet);
    }

    public boolean find(ListNode head, ListNode indexNode, TreeNode root, boolean start, boolean startFinding, Set<TreeNode> chooseSet, Set<TreeNode> noChooseSet) {
        if (start) {
            if (startFinding) {
                if (chooseSet.contains(root)) {
                    return false;
                } else {
                    chooseSet.add(root);
                }
            } else {
                if (noChooseSet.contains(root)) {
                    return false;
                } else {
                    noChooseSet.add(root);
                }
            }
        }
        ...
        if (startFinding) {
            if (indexNode.val == root.val) {
                return find(head, indexNode.next, root.left,false, true, chooseSet, noChooseSet) || find(head, indexNode.next, root.right,false, true, chooseSet, noChooseSet);
            } else {
                return find(head, head, root.left,true, false, chooseSet, noChooseSet) || find(head, head, root.right,true, false, chooseSet, noChooseSet);
            }
        } else {
            if (indexNode.val == root.val) {
                return find(head, indexNode.next, root.left,true, true, chooseSet, noChooseSet) || find(head, indexNode.next, root.right,true, true, chooseSet, noChooseSet)
                        || find(head, indexNode, root.left,true, false, chooseSet, noChooseSet) || find(head, head, root.right,true, false, chooseSet, noChooseSet);
            } else {
                return find(head, head, root.left,true, false, chooseSet, noChooseSet) || find(head, head, root.right,true, false, chooseSet, noChooseSet);
            }
        }
